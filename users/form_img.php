...
if (isset($_POST['btnactualizar'])) {
    ...
    if (move_uploaded_file($_FILES['imagen']['tmp_name'], $target_file)) {
        $sql = "UPDATE users SET imagen = '$target_file' WHERE id = $id";
...
$id = $_GET['id'] ?? null;
if ($id) {
    $stmt = $conexion->prepare("SELECT nombre, imagen FROM users WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $resultado = $stmt->get_result()->fetch_assoc();
    if (!$resultado) die("Doctor no encontrado.");
} else {
...
<div class="container mt-5">
    <h3>Editar Imagen de: <?php echo htmlspecialchars($resultado['nombre']); ?></h3>
    <form action="../users/form_img.php" method="POST" enctype="multipart/form-data">
